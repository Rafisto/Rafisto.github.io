...
const WindowWrap = ({ children, name, maxWidth, invertColor, sx, start, ms, noPadding }: Props) => {
    ...
    const displayName = (name !== undefined)
        ? <Typography className={invtyp} variant="subtitle1" sx={{ fontFamily: "monospace" }}><TypeText start={(start) ? start : undefined} ms={ms}>{name}</TypeText></Typography>
