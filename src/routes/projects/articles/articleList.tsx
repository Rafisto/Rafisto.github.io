...
import { QueryJSON } from "../../../api/data/jsonQuery";
...
const ArticleList = () => {
  ...
  const { isLoading, isError, data } = QueryJSON({
    queryUrl: APILinks.fetchArticles,
  });
        ...
        <TableContainer sx={{ padding: "10px" }}>
          <Grid container rowSpacing={4}>
            <Grid item xs={12}>
              <Typography
                variant="h4"
                style={{ marginBlockEnd: "15px", textAlign: "center" }}
              >
                {T("article.list")}
              </Typography>
              <Typography sx={{ margin: "auto" }} maxWidth="sm">
                {T("article.list.greeting")}
              </Typography>
            </Grid>
            <Grid item xs={8}>
              <Typography
                sx={{ textAlign: "center", transform: "translateY(30%)" }}
                variant="h5"
              >
                {T("article.list")}
              </Typography>
          ...
          >
            ...
            <TableBody>
              {data &&
                data.sort().map((data: ArticleList) => {
                  return data.title
                    .toLowerCase()
                    ...
                    <TableRow
                      key={data.id}
                      onClick={() => articleRedirect(data.url)}
